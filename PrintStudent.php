	...
	if(!isset($_SESSION)){
		...
		if(isset($_SESSION["SESSUserID"]))
		{
			...
			$query11 = "SELECT name as Lastattended 
				FROM stdqual WHERE StdId=" . $_SESSION["SESSUserID"] ." and exam = '12th' LIMIT 1;";
			//echo $query11;
...
<table class="simple-table">
      ...
      <tr>
	    <td colspan="3"><span class="th-heading"><center>Admission Form for <?php 
			if($_SESSION['SESSselectedExamYear'] == 'S.E.')
				echo 'S.Y. B.Tech.';
			else
				echo $_SESSION['SESSselectedExamYear'];
			?> Engineering 
...
<?php
	  ...
	  $query = "SELECT s.StdID as StdId, CONCAT(SUBSTR(EduYear,1,1),'.',SUBSTR(EduYear,2,1),'.') as Exam, 'MKSSS CCOEW' as name, CONCAT(srm.EduYearFr,'-', srm.EduYearTo) as EduYearStart,
				...
				WHERE s.StdId=" . $_SESSION["SESSUserID"] ." 
				...
				remark FROM stdqual WHERE StdId=" . $_SESSION["SESSUserID"] ." order by Exam desc LIMIT $limit;";
