...
?>
	...
	<TR><td colspan='8' class='th-heading'><center><h2>Department-wise Result for Exam: <?php echo $selexamname; ?> (Top 20 Students)</h2></center></td></TR>
	...
	<TR>
		...
		Semetser: <?php echo $_GET['sem']; ?><center></center></td>
...
$prevmarks='';
	...
	include 'db/db_connect.php';
	//if( ($_GET['pattern'] == 2008)  && ($_GET['sem'] == 1) && ($_GET['eduyear'] == 'T.E.') ) {
	if( ($_GET['sem'] == 1) ) {
	
		$sql="SELECT COALESCE(CNUM, SM.UniPRN) AS CNUM, CONCAT(FirstName, ' ', FatherName, ' ', Surname) AS NAME , DM.DeptName,
			  TotalLine AS MksObtInt,
			  SUBSTRING(BLine, LOCATE(':', BLine) + 2) AS Class,  
			  cast(100 * CONVERT(TotalLine, UNSIGNED  INTEGER) /
			  case when '" . $_GET['sem'] . "' = '1' Then 750 else 1500 end AS DECIMAL(6,2))
			  AS TotalPer
			  FROM vwstdresultm SM
			  INNER JOIN tbldepartmentmaster DM ON DM.DeptID = SM.CDept
