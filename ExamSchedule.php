...
// if the form was submitted/posted, update the record
 if (isset($_POST['btnSave']))
	...
	foreach (array('txtPaperID', 'dtEmax','ddlSlot', 'txtFrom', 'txtTo' ) as $pos) {
	...
	for($i = 0 ; $i < $size ; $i++){
			...
			if($num_results)
			{
				//update
				include 'db/db_connect.php';
				$sql = "update tblexamschedule set ExamSlot = ?, ExamDate = ?, TimeFrom = ?, TimeTo = ?, 
												Students = ?,
												Created_by = 'Admin', Created_on = CURRENT_TIMESTAMP, 
												updated_by = 'Admin', Updated_on = CURRENT_TIMESTAMP
											where ExamID = ".$_SESSION["SESSSelectedExam"] . " and 
											PaperID = ". $PaperIDs[$i] ;
				$stmt = $mysqli->prepare($sql);
				//$Blockss[$i], $SupCounts[$i], 
				$stmt->bind_param('ssssi', $ddlSlots[$i], $dt, $txtFroms[$i], $txtTos[$i], $Studentss[$i]);
				if($stmt->execute()){
		...
		$sql = "Update tblexamschedule  set Blocks = ?, SupCount = ?,  ExamSlot = ?, ExamDate = ?, TimeFrom = ?, TimeTo = ?,
					Created_by = 'Admin', Created_on = CURRENT_TIMESTAMP, updated_by = 'Admin', Updated_on = CURRENT_TIMESTAMP 
					where ExamID = ? and PaperID = ? ";
		$stmt = $mysqli->prepare($sql);
		$stmt->bind_param('iissssii', $Blockss[$i], $SupCounts[$i], $ddlSlots[$i], $dt, $txtFroms[$i], $txtTos[$i], 
					$_SESSION["SESSSelectedExam"], $PaperIDs[$i] );
...
?>
<form action="ExamScheduleMain.php" method="post">
	<head>
		<script>
			$(function() {
		...
		<table cellpadding="10" cellspacing="0" border="0" class="tab_split" style="margin-left:5%">
			<tr >
				...
				<td class="form_sec span2">
					<select name="ddlYear" style="width:75%;margin-top:10px" >
...
<option value="FY - Sem 1" <?php echo (isset($_POST['ddlYear']) && $_POST['ddlYear'] == 'FY - Sem 1')?'selected="selected"':''; ?>>FYMTech - Sem 1</option>
<option value="FY - Sem 2" <?php echo (isset($_POST['ddlYear']) && $_POST['ddlYear'] == 'FY - Sem 2')?'selected="selected"':''; ?>>FYMTech - Sem 2</option>
<option value="SY - Sem 1" <?php echo (isset($_POST['ddlYear']) && $_POST['ddlYear'] == 'SY - Sem 1')?'selected="selected"':''; ?>>SYMTech - Sem 1</option>
<option value="SY - Sem 2" <?php echo (isset($_POST['ddlYear']) && $_POST['ddlYear'] == 'SY - Sem 2')?'selected="selected"':''; ?>>SYMTech - Sem 2</option>
				...
				<td class="form_sec span2">
					<select name="ddlPattern" style="width:50%;margin-top:10px">
						...
						<option value="2016" <?php echo (isset($_POST['ddlPattern']) && $_POST['ddlPattern'] == '2016')?'selected="selected"':''; ?> > 2016</option>
						<option value="2015" <?php echo (isset($_POST['ddlPattern']) && $_POST['ddlPattern'] == '2015')?'selected="selected"':''; ?> > 2015</option>
						...
						<option value="2009" <?php echo (isset($_POST['ddlPattern']) && $_POST['ddlPattern'] == '2009')?'selected="selected"':''; ?> > 2009</option>
				...
				<?php
					If ((isset($_POST['btnSearch'])) or (isset($_POST['btnSave'])))
					{
						$sql = " SELECT PM.PaperID, SM.SubjectName, Students, Blocks, SupCount, DATE_FORMAT(str_to_date(ExamDate, '%m/%d/%Y'), '%d-%b-%Y') as ExamDate,
								 ...
								 LEFT JOIN (SELECT * FROM tblexamschedule WHERE ExamID = " .$_SESSION["SESSSelectedExam"]. " )  AS ES ON PM.PaperID = ES.PaperID
								 INNER JOIN tblsubjectmaster SM ON PM.SubjectID = SM.SubjectID
								 INNER JOIN tbldepartmentmaster DM ON DM.DeptID = PM.DeptID
								 WHERE COALESCE(Lecture,0) <> 0 and PM.EnggPattern = '" . $_POST['ddlPattern'] . "' 
						...
						if( $num_results ){
							while( $row = $result->fetch_assoc() ){
								...
								//echo "<td><label style='width:50px' type='text' name='txtStudents[]' class='span6' ///>{$Students}</td>";
