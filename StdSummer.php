<form action="StdSummerMain.php" method="post">
<?php
	if(!isset($_SESSION)){
		session_start();
	}
		...
		if( $num_results ){
			while( $row = $result->fetch_assoc() ){
				...
				echo "</b>&nbsp;&nbsp;&nbsp;&nbsp;<a class='btn btn-mini btn-success' target='_blank'
					href='SummerPrint.php'></i>&nbsp&nbspPrint</a></h4>";
			}
		}

		$query = "SELECT YEAR ,CASE WHEN YEAR = 'F.E.' THEN dm.DeptID ELSE sa.Dept END AS Dept 
					FROM tblstdadm sa 
...
?> 
	...
	<?php
		...
		include 'db/db_connect.php';
		  $sql = "SELECT ExamID,ExamName FROM  tblexammaster em INNER JOIN tblcuryearauto cy ON cy.EduYearFrom  =  em.AcadYearFrom
					INNER JOIN tblstdadm sa ON sa.EduYearFrom = cy.EduYearFrom AND sa.StdAdmID = ". $StdAdmID . "
	...
	<div class="row-fluid" style="margin-left:5%;margin-top:-15px">
		...
		inner join tblexammaster em on em.ExamID = " . $ExamID . " and ys.eduyearfrom = em.AcadYearFrom  
							...
							$sql = "SELECT DISTINCT pm.PaperID , SubjectName ,SUBSTRING(pm.EnggYear,LENGTH(pm.EnggYear),1) as Sem,ys.rowid AS YSID 
									FROM tblyearstructstd yss 
									INNER JOIN tblyearstruct ys ON ys.rowid = yss.YSID 
									INNER JOIN tblpapermaster pm ON pm.PaperID = ys.PaperID AND ys.papertype <> 'TT' 
									INNER JOIN tblsubjectmaster sm ON sm.SubjectID = pm.SubjectID 
									INNER JOIN tblexammaster em ON em.ExamID = " . $ExamID . " AND em.acadyearfrom = ys.eduyearfrom
									...
									ORDER BY pm.PaperID";
							//AND em.Sem = SUBSTRING(pm.EnggYear,LENGTH(pm.EnggYear),1) 
						//echo $sql;
						// execute the sql query
						$result = $mysqli->query( $sql );
						$num_results = $result->num_rows;
						...
						if( $num_results ){
							while( $row = $result->fetch_assoc() ){
								...
								echo "<td class='span2'><a class='btn btn-mini btn-success' href='StdSummerUpd.php?IUD=I&YSID={$YSID}&StdAdmID={$StdAdmID}&sem={$Sem}&ExamID={$ExamID}'>
        ...
        <div class="span5 v_detail">
			...
			<table cellpadding="10" cellspacing="0" border="0" width="90%" class="tab_split">
				...
				<?php
					include 'db/db_connect.php';
					$sql = "SELECT distinct pm.PaperID , SubjectName ,em.Sem,yss.YSID
							FROM tblyearstructstdretest yss
							INNER JOIN tblyearstruct ys ON ys.rowid = yss.YSID
							INNER JOIN tblpapermaster pm ON pm.PaperID = ys.PaperID 
							AND ys.papertype <> 'TT'
							INNER JOIN tblsubjectmaster sm ON sm.SubjectID = pm.SubjectID
							inner join tblexammaster em on ys.eduyearfrom = em.AcadYearFrom  and examname like '%Summer%'
					...
					if( $num_results ){
						while( $row = $result->fetch_assoc() ){
							...
							echo "<td>{$SubjectName} </td>";
							echo "<td class='span3'><a class='btn btn-mini btn-danger' href='StdSummerUpd.php?IUD=D&YSID={$YSID}&StdAdmID={$StdAdmID}&sem={$Sem}&ExamID={$ExamID}'>
