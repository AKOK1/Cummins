...
*/
		$sql = "SELECT EBS.ExamBlockID, TRIM(EBS.StdId)AS StdID,CONCAT(s.SurName, ' ', s.FirstName) AS stdname
				...
				WHERE replace(year,'.','') = '". $EnggYear ."' AND BlockID = ". $_GET['BlockId'] ."  AND ES.ExamDate = '". $_GET['ExDate'] ."'  AND ES.ExamSlot = '". $_GET['ExSlot'] ."'
				AND ES.ExamId = " . $_SESSION['SESSSelectedExam'] . " ORDER BY EBS.ExamBlockID, ExamBlockStdId ";
			//echo $sql;
			...
			?>
					<?php 		
						...
						while( $j < 21 ){
							echo "<TR>";
							echo "<td width='5%'>$j</td><td width='5%'>{$StdSeat[$j-1]}</td><td width='39%'></td><td width='2%'></td>";
							echo "<td width='5%'>$k</td><td width='5%'>{$StdSeat[$k-1]}</td><td width='39%'></td>";
							echo "</TR>";
							$j = $j + 1;
							$k = $k + 1;
						}					
...
<table width="90%" border="0" cellpadding="5" cellspacing="0" class="fix-table" style="margin-left: 5%;">
	<tr>
		<td><center>Name and Signature of Examiner with date</center></td>
		<td width="2%"></td>
		<td><center>Name and Signature of CAP Authority</center></td>
	</tr>
</table>
</body>
</html>
